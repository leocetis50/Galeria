import flet as ft

def main(page: ft.Page):
    page.title="MARAVILLAS DEL MUNDO. (Modernas y Antiguas)"
    page.bgcolor=ft.Colors.BLACK87

    maravillas = [
        {
        "monumento":"Coliseo de Roma",
        "pais":"Italia",
        "año":"70-80 d.C.",
        "descripcion":"Un antiguo anfiteatro romano, cede de espectaculos publicos",
        "imagen":"coliseo.jpg"
        },
        {
        
        "monumento":"Gran Muralla China",
        "pais":"China",
        "año":"Siglo VII a.C. -Dinastia Ming",
        "descripcion":"Un sistema de fortificaciones defensivas de enormes dimenciones",
        "imagen":"muralla-china.jpg"
        
        },
        {

        "monumento":"Chichén Itzá",
        "pais":"Mexico",
        "año":"600 d.C. - 900 d.C",
        "descripcion":"Una ciudad y centro ceremonial de la cultura maya en la peninsula de Yucatan",
        "imagen":"itza.jpg"
        },
        {
        
        "monumento":"Cristo Redentor",
        "pais":"Brasil",
        "año":"1931",
        "descripcion":"Una imponente estatua de Jesucristo en Rio de Janeiro",
        "imagen":"cristo.jpg"
        },
        {

        "monumento":"Machu Picchu",
        "pais":"Peru",
        "año":"Siglo XV",
        "descripcion":"Una ciudadadela inca en lo alto de los andes famosa por sus construcciones de piedra",
        "imagen":"machu-picchu.jpg"
        },
        {

        "monumento":"Petra",
        "pais":"Jordania",
        "año":"Siglo IV a.C.",
        "descripcion":"Una ciudad antigua con estructuras talladas en roca, fue un importante centro comercial",
        "imagen":"petra.jpg"
        },
        {
        
        "monumento":"Taj Mahal",
        "pais":"India",
        "año":"1632-1653",
        "descripcion":"Un mausoleo de marmol blanco, conocido por su arquitectura y bellesa",
        "imagen":"taj_mahal.jpg"
        },
        {

        "monumento":"Gran Piramide de Giza",
        "pais":"Egipto",
        "año":"2580-2560 a.C.",
        "descripcion":"Un monumento funerario y la unica maravilla antigua existente",
        "imagen":"giza.jpg"
        },
        {

        "monumento":"Coloso de Rodas",
        "pais":"Grecia",
        "año":"200 a.C.",
        "descripcion":"Una estatua gigante de bronce de un dios griego",
        "imagen":"coloso-rodas.jpg"
        },
        {

        "monumento":"Jardines colgantes de Babilonia",
        "pais":"Irak",
        "año":"605-562 a.C.",
        "descripcion":"Un complejo de jardines acencionales",
        "imagen":"babilonia.jpg"
        },
        {
        
        "monumento":"Estatua de Zeus en Olimpia",
        "pais":"Grecia",
        "año":"435 a.C.",
        "descripcion":"Una colosal estatua sentada de Zeus",
        "imagen":"zeus.jpg"
        
        },
        {

        "monumento":"Templo de Artemisa en Éfeso",
        "pais":"Turquia",
        "año":"550 a.C.",
        "descripcion":"Un templo dedicado a la diosa Artemisa",
        "imagen":"artemisa.jpg"
        },
        {

        "monumento":"Mausoleo de Halicarnaso",
        "pais":"Turquia",
        "año":"351 a.C.",
        "descripcion":"Un suntuaoso sepulcro para una satrapa persa",
        "imagen":"mausoleo.jpg"
        },
        {

        "monumento":"Faro de Alejandria",
        "pais":"Egipto",
        "año":"280 a.C.",
        "descripcion":"Un gran faro que guiaba a los barcos",
        "imagen":"alejandria.jpg"
        
        },
    ]

    indice_actual=[0]

    contenedor=ft.Container(
        content=ft.Column([]),
        width=400,
        height=500,
        bgcolor=ft.Colors.GREEN_800,
        alignment=ft.alignment.center,
        padding=20
    )

    boton_sigui=ft.ElevatedButton(text="Siguiente Marabilla",
                                  style=ft.ButtonStyle(
                                      bgcolor=ft.Colors.DEEP_PURPLE_ACCENT_200,
                                      color=ft.Colors.LIGHT_GREEN_100
                                  )
                                  )

    def mostrar_monumento():
        maravilla=maravillas[indice_actual[0]]
        contenedor.content=ft.Column([
            ft.Image(src=f"./{maravilla['imagen']}",width=300,height=300,fit=ft.ImageFit.CONTAIN),
            ft.Text(maravilla["monumento"],size=20,weight=ft.FontWeight.BOLD),
            ft.Text(f"Pais: {maravilla['pais']}",size=16),
            ft.Text(f"Año: {maravilla['año']}",size=16),
            ft.Text(maravilla["descripcion"],size=14,italic=True)
        ],alignment=ft.MainAxisAlignment.CENTER)

        if indice_actual[0]== len(maravillas)-1:
            boton_sigui.text="Volver al inicio"
        else:
            boton_sigui.text="Siguiente Maravilla"
        page.update()

    def siguiente_click(e):
        indice_actual[0]=(indice_actual[0]+1)%len(maravillas)
        mostrar_monumento()
    boton_sigui.on_click=siguiente_click    


    page.add(
        ft.Container(
            content=ft.Column([
                contenedor,
                boton_sigui
            ],alignment=ft.MainAxisAlignment.CENTER,
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            spacing=20
        ),
                alignment=ft.alignment.center,
                expand=True
        )
    )
    mostrar_monumento()

ft.app(target=main, assets_dir="assets")